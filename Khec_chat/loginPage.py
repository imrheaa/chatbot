...
def chatbot_response(text):  
    ...
    if prob > 0.80:
        ...
        if l!=0:
            ...
            mydb.commit()
            if val==2:
                messagebox.showerror("Error", "We will get back to you...Same question don't ask now!!!")
            elif val==3:
                mycursor = mydb.cursor()
                mycursor.execute(f"UPDATE `new_user_regis` SET `status`= 0 where `email`='{emailLogin}'")
                base.destroy()
                main_account_screen()
                mydb.commit()
        else:
    ...
    if emailVal=="Place your registered email...":
        ...
        if len(list1)!=0:   #email register is there...it may have status 0 or 1
            ...
            if status==0:  #status==0
                ...
                messagebox.showerror("Blocked As Spammer!!!", "*This email is block by admin...")
                root.destroy()
                main_account_screen()
            else:    #status==1
                #successful login
